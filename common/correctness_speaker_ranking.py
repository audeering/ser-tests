r"""Test correctness of speaker ranking.

If we rank speakers by their average value
they should match the actual ranking,
especially at the top and bottom of the scale.
"""
import operator
import os
import typing
...
from common.metrics import (
    spearmanr,
    top_bottom_confusion,
    top_bottom_precision,
)
from common.comparisons import abs_gt
from common.plots import plot_ranking
...
def run(
    ...
    for database in databases:
        ...
        # Filter for relevant available labels and speaker labels
        df['speaker'] = db[speaker_table][speaker_column].get(index=df.index)
        df = df.dropna(subset=['speaker', column])
        truth = df[column]
        y = y.loc[truth.index]
...
def load_config(condition):
    ...
    * 'plot' - if the metric is returning an image instead of a value
    ...
    for metric in metrics:
        ...
        if 'Spearmans Rho' in metric['name']:
            if condition in CATEGORY_LABELS:
                metric['function'] = (
                    lambda truth, prediction: {
                        class_label: spearmanr(
                            truth[class_label], prediction[class_label])
                        for class_label in truth.columns
                    }
                )
            else:
                metric['function'] = spearmanr
            metric['comparison'] = abs_gt

        elif 'Precision Top 25%' in metric['name']:
            if condition in CATEGORY_LABELS:
                metric['function'] = (
                    lambda truth, prediction: {
                        class_label: top_bottom_precision(
                            truth[class_label],
                ...
                metric['function'] = (
                    lambda truth, prediction:
                    top_bottom_precision(truth, prediction, 'top')
                )
            metric['comparison'] = operator.gt

        elif 'Precision Bottom 25%' in metric['name']:
            if condition in CATEGORY_LABELS:
                metric['function'] = (
                    lambda truth, prediction: {
                        class_label: top_bottom_precision(
                ...
                metric['function'] = (
                    lambda truth, prediction:
                    top_bottom_precision(truth, prediction, 'bottom')
                )
            metric['comparison'] = operator.gt

        elif 'Top-Bottom Confusions' in metric['name']:
            if condition in CATEGORY_LABELS:
                metric['function'] = (
                    lambda truth, prediction: {
                        class_label: top_bottom_confusion(
                            truth[class_label],
                            prediction[class_label],
                        )
                        for class_label in truth.columns
                ...
                metric['function'] = top_bottom_confusion
            metric['comparison'] = operator.lt

        elif metric['name'] == 'Visualization':
            metric['function'] = (
                lambda truth, prediction:
                plot_ranking(truth, prediction, 'speaker')
            )
            metric['plot'] = True
